...
| 方法 | 入力・操作 | 備考 |
|------|------------|------|
| **① 操作アイテム** | 入場時に時計（`ROBW:menu`）または **`robw:control`** を自動配布。無いときは `/scriptevent robw:give_wand` | ブロック右クリックでも可 |
...
**注意:** スクリプト変更はワールド**再入場**（できればゲーム再起動）後にゲームへ反映される。監視はコピーまで自動。
3. 診断: **`/function robw/ping`** → チャットに `[ROBW] function load OK` なら関数登録は成功
4. 本番: **`/function robw/menu`** または **`/scriptevent robw:menu run`**
5. ゲーム 1.21.80 以降なら **`/robw:menu`**（Content Log に `registered slash commands`）
...
5. 「納品チェストを足元に設置しました」の座標に捕獲アイテムを入れる → +1pt / -2pt（毛皮は消費される）
