...
1. ゲート起動（時計 `ROBW:start` 等）→ 全員招集・**納品チェスト1つ自動設置**（周囲の既存チェストは撤去）・骨配布・オオカミ出現
...
```javascript
BOX_GATE: { x: 0, y: 86, z: 0, radius: 3 },           // 招集先
SUBMISSION_CHEST: { x: 0, y: 85, z: 0 },            // フォールバック（通常は起動時に自動配置）
CHEST_FLAT_SEARCH_RADIUS: 24,                     // 平坦な設置場所の探索半径
